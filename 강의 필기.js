...
normal   :	연속된 공백은 하나의 공백으로 채워진다.(스페이스 두번쳐서 띄어쓰기 두번해도 그냥 하나의 빈칸으로 표시된다는 말) 가로의 오른쪽 끝에 닿으면 자동으로 줄 바꿈 한다.
nowrap   :	공백을 채우지만 가로 폭의 오른쪽 끝에서 줄 바꿈을 하지않고 박스 벗어나 표시된다.
pre      :	연속된 공백을 그대로 표시한다. 또한 긴 줄도 줄 바꿈하지 않고 표시한다.
pre-wrap :	연속 공백은 그대로 유지된다. 행(行)의 줄 바꿈은 행의 박스를 채우기 위해 필요한 경우에 실행합니다.
pre-line :	연속된 공백은 하나에 공백으로 표시됩니다. 행의 줄 바꿈은 행의 박스를 메꾸기 위해 필요한 때에 실행됩니다.
